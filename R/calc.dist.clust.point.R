...
function(clust, pnt, type = "single", dist.type = "spearman"){
	...
	size <- ncol(clust)
	if (size == 1){
		if (is.element(dist.type , c("pearson", "spearman"))) return(1 - abs(cor(clust, pnt, use = "everything", method = dist.type))) else
	 		if (dist.type == "euclid") {
	...
	distances <- apply(clust, 2, function(x){
		if (is.element(dist.type , c("pearson", "spearman"))) return(1 - abs(cor(x, pnt, use = "everything", method = dist.type))) else
			if (dist.type == "euclid") {
